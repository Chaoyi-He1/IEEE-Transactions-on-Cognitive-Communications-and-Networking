...
timescales of microseconds. This mismatch is five or more orders of
magnitude and is the primary source of spectrum under-utilization as repeatedly
...
sensing algorithms with the ability to predict. Third, we present a complete
...
granularity of a single OFDM frame (approximately 13~$\mu$s), four to six
orders of magnitude finer than the methods surveyed.
...
current state does not contain, and that information is only realized in the
...
access systems grants and revokes rights on demand \cite{FCC2015CBRS};
...
they operate within the spectrum bands an access system grants, where these
bands remain unavailable for seconds, minutes, or longer (see Fig.~\ref{fig_0}).
...
In an IEEE 802.11ax network, the atomic unit of transmission
...
\caption{The gap this paper addresses. Above: an 80~MHz band at the
granularity of a single OFDM frame across eight 10~MHz sub-blocks, shaded by
received power; the column at right gives the occupancy a single sensing
...
sub-blocks as occupied; at frame resolution, fewer than one fifth of the
time--frequency cells carry energy.}
\label{fig_0}
\end{figure*}
%------------------------------------------------------

Fig.~\ref{fig_1} contrasts the two regimes.
...
device---a radar return pulse, a satellite uplink or a sensor report---can occupy
...
On the left, various base stations (BS1, BS2, BS3) and devices (including primary
users PU1 and PU2 and secondary users SU1 and SU2) communicate via satellite links.
...
resulting in inefficiencies. In contrast, the right section shows the proposed system,
...
Three capabilities must be distinguished, and the literature does not
...
and wavelet methods all address this problem, and they address it well
...
a) The required model should consume raw complex baseband
...
Those cues survive in the I/Q stream.
The cost of this commitment is severe: an 80~MHz band sampled at
...
Any waveform-level predictor is therefore, first, a data-reduction problem,
making representation learning not an optional refinement of the architecture
but a load-bearing component.

b) The model should be \emph{generative} rather than discriminative.
...
waveform, and a regression trained to minimize squared error against the
realized future will converge toward the conditional mean, which for a
multimodal predictive distribution is a waveform that could not
physically occur.
As opposed to the above, a generative model draws samples from the
predictive distribution.
Each sample represents a physically plausible future;
their spread provides a calibrated statement about uncertainty; and
downstream, the disagreement among samples about whether a sub-block
will be occupied is precisely the quantity a secondary user needs in
...
material in Sections~III and IV is presented accordingly, including:
...
modeling---that bound what any given method can do. We use this
organization to locate precisely where the granularity gap originates
in the existing literature.

\textbf{\emph{A tutorial treatment}} of the enabling machine learning
(Section~IV), covering contrastive predictive coding and denoising
diffusion at a level sufficient for a wireless engineer to assess
design choices, together with an analysis of the properties of OFDM
waveforms---cyclic prefix structure, I/Q representation, symbol-level
symmetry---that require these general-purpose tools to be adapted.

\textbf{\emph{A reference architecture}} (Section~V) that realizes
sub-frame anticipation as three composable stages, with each stage's
role, interface, and cost stated explicitly so that alternatives can be
substituted.
...
is exact by construction. We argue for transition-conditioned
evaluation, sensed-persistence baselines, and reporting at a
collision-referenced operating point, and we quantify how much
apparent performance these corrections remove.
...
the case study is partly negative: the generative stage alone does not
outperform a well-constructed conventional baseline. We report it
prominently because we believe the field is at a stage where honest
accounting of what generative models do and do not contribute is more
valuable than another incremental score, and because the accompanying
positive finding---that generation and sensing are strongly
complementary---is only interpretable alongside it.

\textbf{\emph{Design principles and a research agenda}}
(Sections~VIII and IX), including a latency budget that quantifies
...
Section~II characterizes spectrum occupancy as an object of
measurement and modeling, and establishes the timescale ladder on which
...
we mean by prediction, and more specifically, what is being predicted.
This is necessary because the literature's implicit answers to that
question differ more than its stated methods do.

\subsection{\textbf{Dimensions of Occupancy}}
Occupancy is not represented by a scalar, but by a vector.
A band may be occupied in time, in frequency,
in space, in code, and in angle of arrival, and a resource is genuinely
available to a secondary user only if it is free in every dimension that
the secondary user's own transmission would consume \cite{Yucek2009}.
...
Occupancy remains highly heterogeneous across bands and directions.
...
uplink, and the Citizens Broadband Radio Service and 5G n77 bands stand
out as markedly less occupied than other cellular bands studied
\cite{Raouf2023}.
...
its comparability across campaigns \cite{Wellens2010}.
...
reflects a particular labeling convention, not physical occupancy.
Waveform-level methods do not eliminate this problem---labels
...
In the corpus used in this paper, occupancy dwell times fall in
...
\begin{table}[t]
\centering
\caption{The Spectrum Decision Timescale Ladder}
...
$10^{-3}$~s  & Packet/TXOP            & MAC contention \\
$10^{-5}$~s  & OFDM frame/symbol      & PHY \\
\bottomrule
\end{tabular}
\end{table}
%------------------------------------------------------------------------------
...
\label{sec:nested}
One structural property of the occupancy signal is easily overlooked and
materially affects how results should be assessed.
Standards constrain allocation to be contiguous and quantized: in 802.11ax,
the band is divided into 20~MHz increments, so the eight 10~MHz occupancy
indicators of an 80~MHz channel are not eight independent binary variables.
...
This has two consequences: (a) reported per-sub-block accuracies are
inflated relative to what an independent-label reading would suggest,
because correctly predicting the aggregate occupancy level determines
most of the individual labels, and (b) any metric must be interpreted
...
spectrum prediction.
We organize the survey by the three structural choices identified in Section~I:
...
according to empirically fitted transition probabilities, with sojourn
times drawn from fitted holding-time distributions
\cite{Gibson1994, Percival1997}.
Refinements conditioned on the previous
state and the adjacent channel state yield eight transition
probabilities rather than four \cite{Saad2016, Pan2026}.
Where the underlying process is naturally identified, discrete-time chains are
more appropriate.

From a prediction standpoint, the practical formulation is a hidden
...
for instance as maximum-likelihood classification over state sequences
...
The strength of this family of prediction algorithms is that it models the
stochastic process explicitly---the third of the three structural
choices---and it is the only classical family that does.
...
Model selection follows the standard Box--Jenkins diagnostic. Writing
the model as ARIMA($p,d,q$) with $p$ autoregressive lags, $d$ orders of
differencing, and moving-average window $q$, one inspects the
autocorrelation function (ACF) and partial autocorrelation function
(PACF).
An AR structure is indicated when the PACF cuts off and the ACF
tails off, MA when the reverse holds, ARMA when both tail off, and ARIMA
when the ACF fails to decay, and differencing is required.
...
Deep sequence (LSTM, CNN, ConvLSTM, hybrids) &
...
From roughly 2010, when machine learning methods entered the field, their
principal advantage is that they require no prior structural
knowledge of the occupancy process \cite{Tumuluru2010}.
The multilayer perceptron (MLP) is the simplest such predictor; it is
observed in \cite{Yin2011} that ARIMA is essentially a restricted case
of the MLP, and that input normalization materially improves the MLP's
generalization across frequencies and services.
MLPs have been applied to GSM \cite{Yin2011} and television broadcast bands
...
The common limitation of this generation of methods is dependence on feature
...
networks \cite{Jia2020} enabled data-driven prediction across
spatio-temporal-spectral domains.
A practical distinction emerged: LSTM and recurrent architectures have
...
features, and reinitializing the final layer for the new band
\cite{Li2023}.
Because occupancy statistics differ across signal types,
transfer component analysis is used to verify band similarity before
transfer.
...
These approaches all assess first and predict second. A more integrated strategy
embeds assessment in the predictor, as in GRU-D, LSTM-M,
...
Because self-attention does not propagate state sequentially, it retains
...
spatiotemporal prediction \cite{Pan2026}.
Singular spectrum analysis, which decomposes
...
MSTFformer \cite{Li2025} and CNN-Transformer architectures
...
resolves into a consistent picture, summarized in
Table~\ref{tab:taxonomy}.

\emph{\textbf{Observable.}} With few exceptions, the entire literature consumes
aggregated power measurements, duty cycles, or binary occupancy labels.
This choice is what caps temporal resolution, because these observables
are only meaningful when integrated over an interval much longer than a
symbol.
...
\cite{Chirov2023} typical, and frequency granularity is usually the full
...
express sub-block structure. The deep learning literature predicts a
point estimate of a future occupancy value; it does not represent the
...
requires changing all three at once. That is what the architecture of
Section~V attempts.
...
Consider the arithmetic. An 80~MHz band captured as complex baseband at
80~MHz produces $8\times10^{7}$ complex samples per second. The
architecture of Section~V conditions on 128 frames of history, each of
1024 complex samples---roughly $1.3\times10^{5}$ complex values, or
$2.6\times10^{5}$ real values, per prediction. Feeding this directly to
a generative model is impractical at any deployment scale, and it is
also statistically wasteful: the informative content of the history for
the purpose of predicting occupancy transitions is far
lower-dimensional than the raw stream.

The task is therefore to learn a mapping from the history to a compact
...
reconstruction wastes capacity on high-frequency detail---in our setting,
on the specific payload bits and the specific noise realization---that is
...
The model has two components. An encoder $f_{\mathrm{enc}}$ maps each
input context window $x_t$ to a latent embedding in a fixed-dimensional
space,
\begin{equation}
z_t = f_{\mathrm{enc}}(x_t),
\end{equation}
and an autoregressive aggregator $g_{\mathrm{ar}}$ summarizes the last
...
the contrastive objective below.
...
$p(x_{t+k} \mid c_t)$ and $N-1$ negatives drawn from the marginal
$p(x_{t+k})$.
...
point, since it is frequently overstated in applied work: the objective
optimizes a bound, not the mutual information itself, and the tightness
of that bound depends on the number of negatives.

The choice of negatives is where domain knowledge enters, and it is the
...
environment-agnostic structure---the traffic and allocation dynamics
that are actually shared.

%--------------------------------------------------------------------
\subsection{\textbf{Denoising Diffusion Probabilistic Models}}
Diffusion models \cite{SohlDickstein2015, Ho2020} generate samples by
learning to invert a fixed noising process.
Relative to the two alternatives a wireless reader is most likely to
have encountered, they offer a favorable trade: variational autoencoders
\cite{Kingma2014} are stable but tend to produce over-smoothed samples,
which is precisely the failure mode that defeats the purpose here; generative
adversarial networks \cite{Goodfellow2014} produce sharp samples but are
unstable to train and prone to mode collapse, which for a predictor of a
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Fig/fig_ofdm_adapt.pdf}
...
Both kernel configurations studied here exceed that requirement by a wide
margin. With strides [3,3,3,3,3] and dilations [5,5,5,3,3], the small kernel set
[7,7,9,9,11] gives a final-layer receptive field of 3,559 samples and the
large set [513,385,257,129,65] gives 45,761, against a frame length of 1024.
...
precisely the inductive bias a convolutional stack encodes by construction
...
no function of the history that returns the correct future; there is only
a distribution over futures.
A generative model represents that
distribution, and the spread of its samples is the honest expression of
what is knowable.
This is the conceptual justification for the entire
architecture, and it also sets the ceiling on achievable performance:
no predictor can do better than the entropy of the arrival process
permits.

%======================================================================= FIG. 4
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Fig/Slide2.pdf}
...
classifier maps each to sub-block occupancy. Four history frames are
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Fig/Fig_arch.pipeline}
...
conditioned on $\mathbf{z}_t$; the temporal-to-frequency classifier maps each
...
$T_{\mathrm{hst}} = 128$ history frames to a 128-dimensional context
vector. Its convolutional backbone uses centered kernels with stride
...
deployed configuration uses $T_{\mathrm{pred}} = 32$ frames, or
approximately 410~\us; this horizon was set by available GPU memory
...
The temporal-to-frequency (T2F) classifier maps each generated waveform
to the occupancy of eight 10~MHz sub-blocks per frame. It is a 1D
convolutional network of 17M parameters, pretrained on ground-truth
waveforms.
...
and the realized one. A weighted cross-entropy focuses gradient on
errors:
\begin{align}
...
in spectrum prediction---compute AUC, accuracy, or F1 over randomly drawn
...
\subsection{The Dwell-Time Problem}
...
\subsection{Transition-Conditioned Evaluation}
...
precision. An AP of 0.90 against a base rate of 0.69 is a modest result,
and reporting AP without the base rate invites it to be read as a strong
one.

\subsection{Baselines That Deserve the Name}
...
\subsection{Operating Points and the Metric That Matters Operationally}
...
%================================================================== SECTION VII
\section{Case Study: An 802.11ax Realization}

\subsection{Corpus Construction}
\label{sec:corpus}
The corpus simulates a dynamically changing indoor environment in which
multiple users share a fixed channel through a single transmitter. The
...
generation pipeline is summarized in Fig.~\ref{fig:datagen}.
...
\subsubsection{User arrivals and payloads}
...
\subsubsection{Channel and impairments}
...
% break \includegraphics. Rename to Testing_errors.png (as above) or load grffile.
...
\subsection{Classifier, System, and Ablations}
...
\subsubsection{System-level results}
...
\subsubsection{Inference-time ablations}
...
metrics and, we would argue, the single most important number in the
table for anyone considering deployment.
...
\label{fig_res8}}
\vfil
\subfloat[]{\includegraphics[width=\textwidth]{Fig/Slide5c.pdf}%
\label{fig_res9}}
...
\subsubsection{Position relative to prior work}
Table~\ref{tab:comparison} places the architecture against representative
...
\subsection{Zero-Shot Transfer to Unseen Environments}
\label{sec:zeroshot}
...
\subsection{Over-the-Air Demonstration at Packet Granularity}
\label{sec:ota}
...
devices and routers. Successful reception required an error vector magnitude
below $-10$\,dB; typical packets measured $-30$\,dB.

\begin{figure}[t]
\centering
\includegraphics[width=3.4in]{Fig/measurement_setup}
...
\subsubsection{Dataset and retraining}
...
pipeline. Table~\ref{tab:ota} reports the results.
...
; generalization is assessed instead on the simulated
...
\subsection{Generation Complements Sensing; It Does Not Replace It}
...
\subsection{Evaluate Where the Event Occurs}
...
\subsection{Match Architecture to the Locality of the Label}
...
\subsection{Representation Is the Transfer Bottleneck}
...
\subsection{Inference-Time Choices Can Erase Model-Time Gains}
...
\subsection{What Sub-Frame Resolution Buys}
...
%=================================================================== SECTION IX
\section{Open Challenges and a Research Agenda}
...
\subsection{Impairments, Mobility, and Environment Population}
...
\subsection{Horizon and the Entropy Limit}
...
\subsection{Standards and Protocol Integration}
...
\subsection{Toward General Radio-Frequency Representation Models}
...
this manuscript; and Sanjay Patel and Mariea Sharaf Anzum of Texas A\&M
