...
sensing algorithms capable of predicting. Third, we present a complete
...
they operate within the spectrum bands \red{\st{an access system grants} granted by an access system}, where these
bands \red{\st{remain} are assigned to be} unavailable for seconds, minutes, or longer (see Fig.~\ref{fig_0}).
...
\red{\st{On the left, various base stations (BS1, BS2, BS3)and} The top section shows various devices (including primary
...
resulting in inefficiencies. In contrast, the \red{\st{right} red} section shows the proposed system,
...
\red{Spectrum occupancy prediction using AI/ML is typically framed as a binary classification problem: \textit{Is a channel idle or occupied?}. In literature, a model's performance is most commonly assessed using a mix of well-established metrics from the AI/ML field like accuracy, F1 score, and area under the ROC curve (AUC). The F1 score, defined as the harmonic mean of precision and recall, is useful for assessing spectrum prediction methods under class imbalance, such as scenarios where spectrum occupancy is infrequent relative to idle periods. A high F1 score reflects that a sensing or prediction strategy performs well both at detecting occupied bands (recall) and at avoiding false alarms (precision). AUC complements F1 as a threshold-independent measure that summarizes a model's overall ability to separate occupied from idle states across all decision thresholds, rather than at a single fixed operating point. These two metrics accompany accuracy in literature since accuracy alone can be misleading on class-imbalanced dataset. However, we argue that these metrics become misleading as we go down the timescale ladder to predict occupancy at the granularity of frames. Section VI addresses this.}
...
%----------------------------------------------------------------------------------SUBSECTION IV-C
\subsection{\textbf{Denoising Diffusion Probabilistic Models \red{(DDPM)}}}
...
the are two alternatives that are faced: either coherent processing to 
...
leading to failure mode or generative
...
no predictor can do better than the entropy \red{\st{of} that} the arrival process
permits.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=5.5in]{Fig/Fig_arch.pipeline}
...
deployed configuration uses $T_{\mathrm{pred}} = 32$ OFDM symbol intervals, or
 409.6~\uswheneach interval contains 1024-sample useful symbol at 80 MHz.
...
convolutional \red{neural} network \red{(CNN)} of 17M parameters, pretrained on ground-truth
waveforms. \red{In Section VII.D, we demonstrate that using the classical CNN for the T2F stage outperforms more modern and advanced attention-based models.}
...
in spectrum prediction---compute AUC, accuracy, or F1 \red{(Section III.G)} over randomly drawn
...
precision \red{(AP)}. An AP of 0.90 against a base rate of 0.69 is a modest result,
...
For the appropriate dataset partitioning, we considered the relationship among simulated environments as shown  
\red{in \st{on Fig. ~\ref{fig:Figure_Data_Part}} Fig. \ref{fig:fig_sysdiag}}. Independent waveform realizations, payload partitions, component-specific training sets, validation sets, and final evaluation sets. Environments and payload sequences assigned to final testing are excluded from model training and hyperparameter selection. The transition-conditioned set is derived only after the final test split is fixed.The
...
\caption{The dataset partitioning was based on the relationship among simulated environments,
independent waveform realizations, payload partitions, component-specific training sets, and final evaluation sets. Environments and data sequences assigned to the final testing were excluded from model training and hyperparameter selection.}
...
%-----------------------------------------------------------------------------------------SUBSECTION VII-B
\subsection{\textfb{Representation Learning}}
...
metrics and, we would argue, the single most important number in \red{\st{the
table}} \red{Fig. \ref{fig:Figure_Operation_vacancy.pdf}} for anyone considering deployment.
...
\caption{Operating-regime map of representative spectrum-occupancy prediction methods.The horizontal axis shows the prediction interval per forecast step on a logarithmic scale, while the vertical axis identifies the input representation. Marker shape distinguishes deterministic point prediction from stochastic or generative prediction, and marker color denotes the frequency granularity of the predicted output. The proposed method operates in a distinct regime characterized by raw-waveform input, symbol-scale prediction, stochastic generation, and standardized sub-block occupancy. The studies are summarized in Table VII; their datasets and evaluation protocols differ, so the figure compares operating regimes rather than predictive performance.
}
...
\label{fig_res8}}
\\[2mm]
\subfloat[]{\includegraphics[width=0.85\textwidth]{Fig/Slide5c.pdf}%
\label{fig_res9}}
...
Table~\ref{tab:comparison} \red{and Fig. \ref{fig:Figure_Literature.pdf}} places the architecture against representative
...
devices and routers. \red{To control dataset quality} successful reception required an error vector magnitude \red{(EVM)}
below $-10$\,dB; \red{for comparison} typical packets measured \red{EVM of} $-30$\,dB.
...
\caption{Predicted sub-band occupancy on 512 consecutive received packets (Test~1) for the fusion configuration: the 20-sample ensemble confidence of the pipeline fused (geometric mean) with the T2F classifier's estimate from the 32 most recent packets, thresholded at 0.5. (a) Ground truth; (b) prediction; (c) error classes. Decision accuracy 0.94 (T2F sensing alone 0.93; generative pipeline alone 0.73); errors are confined to the prediction window immediately following each occupancy change.}
...
Sub-block occupancy accuracy (best)    & 0.828 \\. Fig.~\ref{fig:occupancy_measured} shows the resulting prediction on 512 consecutive received packets for the fusion configuration; its errors are confined to the prediction window immediately following each occupancy change.
F1 score                               & 0.899 \\
...
so these figures characterize in-sample fit on the recorded dataset rather than \red{performance on unseen, out-of-sample data}; generalization is assessed instead on the simulated
