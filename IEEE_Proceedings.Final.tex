...
pipeline. Table~\ref{tab:ota} reports the results. For the visualization of Fig.~ef{fig:occupancy_measured} we apply the fusion configuration of Section~ef{sec:system_results} to the measured stream, with the sensed estimate taken from the 32 most recent packets rather than the full 128-packet history: at the packet level, occupancy changes only every few hundred packets, and averaging the sensed estimate over the whole history delays the response to each change by tens of packets. The generative pipeline alone reaches a decision accuracy of 0.73 on this stretch, sensing alone 0.93, and their fusion 0.94, with errors confined to the prediction window immediately following each occupancy change.
