...
\begin{table*}[ht]
\centering
\caption{Stage ablations on the 135 validation windows whose 32-frame future contains an occupancy flip (seed 42; occupied base rate 0.690; bootstrap standard errors $\approx 0.01$). Transition-free windows ($>$99\% of the corpus) are excluded because there any persistence-style predictor is nearly exact (T2F-sensed persistence reaches 0.99 AUC, and copying ground-truth history labels is exact by construction; on the transition windows the latter scores 0.8323); the values in Sections~\ref{sec:sys_eval} and~\ref{sec:zeroshot} refer to randomly drawn windows and are not comparable. The two sensed-persistence rows copy forward a per-sub-block occupancy estimate obtained from the 128 history frames, by an FFT band-power detector or by the T2F classifier; the state-conditioned variant appends that 8-dimensional estimate to the diffusion context (30-epoch adaptation of the diffusion stage only); the fusion row takes the geometric mean of the proposed system's ensemble confidence and the sensed estimate, exceeding sensed persistence by $+0.034 \pm 0.008$ AUC and $+0.044 \pm 0.007$ AP ($P>0.999$), while a fusion built on the direct-regression head instead reaches an indistinguishable 0.8861 at far lower latency. AP: average precision (no-skill floor 0.690). Vacancy det.: fraction of vacant sub-blocks identified at the threshold that declares 1\% of occupied sub-blocks vacant. Latency: median single inference on one NVIDIA A100.}
...
Sensed persistence (T2F on history)
  & \textit{none} & 0.8499 & 0.9031 & 12.8\% & 43 \ul{ms} \\
\hline

Encoder $\rightarrow$ CNN, direct label regression
  & \textit{removed} & 0.8382 & 0.9237 & 5.7\% & 20 \ul{ms} \\
\hline

Proposed (20 samples, ensemble mean)
  & full & 0.8185 & 0.9174 & 6.0\% & 8025 \ul{ms} \\
\hline

Proposed, state-conditioned diffusion
  & full & 0.8301 & 0.9233 & 2.9\% & 8068 \ul{ms} \\
\hline

\textbf{Proposed $\oplus$ sensed state (fusion)}
  & \textbf{full} & \textbf{0.8844} & \textbf{0.9473} & \textbf{12.8\%} & \textbf{8068 \ul{ms}} \\
\hline
...
\begin{table}[ht]
\centering
\caption{Inference-time ablations on the saved checkpoint under a random validation draw (60 windows per environment, seed 42, $N{=}1800$; proposed configuration AUC 0.7612 — a protocol distinct from Table~\ref{tab:comparison}). The fixed-threshold majority vote is degenerate because nearly every per-sample confidence exceeds 0.5; ranking requires the ensemble-mean confidence or a swept threshold as in Fig.~\ref{fig_res6}.}
\label{tab:ablation}
