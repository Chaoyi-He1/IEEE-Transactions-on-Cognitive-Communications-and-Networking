...
\begin{table*}[ht]
\centering
\caption{\red{Stage Ablations - all evaluated on the same validation split and 32-frame horizon.}}
...
\textbf{Proposed (20 samples, majority vote)}
  & \textbf{full} & \textbf{0.8388} & \textbf{[XX]} & \textbf{[XX]\%} & \textbf{[XX] \ul{ms}} \\
\hline

Proposed, zero-shot (Sec. V-E)
  & full & 0.7589 & {[XX]} & {[XX]\%} & {[XX] \ul{ms}} \\
\hline

\textit{T2F on ground-truth waveforms (upper bound)}
  & \textit{oracle} & \textit{[XX]} & \textit{[XX]} & \textit{[XX]\%} & --- \\
\hline
...
\begin{table}[ht]
\centering
\caption{\red{Design ablation study.}}
\label{tab:ablation}
...
Encoder kernels
  & causal vs. centered & {[XX]} & {[$\pm$XX]} \\
\hline

History length \ul{T\_hst}
  & 64 / 128 / 256 frames & {[XX]} & {[$\pm$XX]} \\
\hline
...
Fine-tuning weight (Eq. 7)
  & 0.1 vs. 1.0 & {[XX]} & {[$\pm$XX]} \\
\hline

Sampler
  & DDPM (N steps) vs. DDIM (k steps) & {[XX]} & {[$\pm$XX]} \\
\hline

Ensemble aggregation
  & majority vote vs. mean probability & {[XX]} & {[$\pm$XX]} \\
\hline
