...
The system is validated across 30 simulated channel realizations under the IEEE 802.11ax (WiFi 6) standard. Over a 32-frame horizon, and evaluated on the windows in which sub-channel occupancy changes, fusing the generated futures with a sensed estimate of the current state attains an area under the ROC curve (AUC) of 0.884 for spectrum occupancy classification, approaching the 0.897 bound of the classifier applied to the true future waveform. These results demonstrate that the proposed framework can provide the fine-grained, real-time spectrum awareness needed to support simultaneous multi-system coexistence within shared frequency bands — a key step toward alleviating spectrum congestion in next-generation wireless networks \cite{intro_ref17, intro_ref18, intro_ref19}.
...
In the deployed system, this aggregated context vector is the conditioning signal supplied to the diffusion model of the following subsection; for consistency with Fig.~\ref{fig_model_1} it is written $z_t$ in the remainder of the paper — the symbol $f$ in the diffusion subsection denotes the same vector — while the per-window embeddings $f_{enc}\left(x_t\right)$ enter only the contrastive objective below. The deployed configuration uses $T_{hst}=128$ history frames and a latent space vector length of 128.
...
\begin{aligned}
    ...
    &\propto\frac{p\left(x_{t+k}\middle| c_t\right)}{p\left(x_{t+k}\right)}
\end{aligned}
\end{equation}
...
At the optimum of the InfoNCE objective, $M_{\theta_k}$ is proportional to the density ratio $\frac{p\left(x_{t+k}\middle| c_t\right)}{p\left(x_{t+k}\right)}$; minimizing the loss below maximizes a lower bound on the mutual information $I\left(x_{t+k};c_t\right)$ \cite{model_ref1}, rather than the mutual information itself.
...
\begin{equation}
    \begin{aligned}
        ...
        &=p\left(y_t^{\left(N\right)}\right)\prod_{n=1}^{N}\mathcal{N}\left(y_t^{\left(n-1\right)};\mu_\theta\left(y_t^{\left(n\right)},n,f\right),\beta_nI\right)
    \end{aligned}
\end{equation}
...
In this work, we employ a contrastive learning method that enables our prediction system to operate effectively across a wide range of wireless communication environments. Unlike previous state-of-the-art spectrum prediction models focusing on specific frequency bands or static environments, our encoder adaptively compresses and extracts features from diverse and dynamic signal environments. This flexibility suggests that the model may generalize to new wireless environments with limited or no retraining, a property that is examined for the diffusion stage in Section~\ref{sec:zeroshot}; because the encoder and the T2F classifier are held fixed in that experiment, it isolates the transfer behavior of the generative stage, and joint transfer of all three components, as well as few-shot adaptation, remains a direction for future work \cite{result_ref1, result_ref2, result_ref3}.
...
The ROC curve highlights an optimal global threshold that balances precision and recall. Applying a majority vote introduces a trade-off between inference speed and final accuracy—more sampling iterations can enhance accuracy but may slow down the overall inference process. Additionally, the diffusion model's configuration can be tailored for faster inference or improved denoising reliability, depending on the sampling method (e.g., DDIM~\cite{Song2021DDIM} or DDPM~\cite{Ho2020DDPM}), the time step size in the reverse diffusion process, and the number of time frames generated; all results reported in this paper use DDPM sampling. This flexibility demonstrates the model's adaptability to various operational requirements, ensuring speed and accuracy in spectrum utilization forecasting.

Two properties of the data shape how these curves should be read. First, occupancy dwell times (0.1--1~s) far exceed the 410~$\mu$s horizon, so a randomly drawn window almost never contains an occupancy change: in a seeded re-evaluation of the trained system on 60 windows per environment ($N{=}1800$), not one window contained a transition inside its 32-frame future, the global AUC re-measured at 0.766, and a predictor that simply copies the current occupancy forward is exact by construction. Second, the ROC of Fig.~\ref{fig_res6} was obtained on an earlier, unseeded draw of windows that is no longer recoverable; the seeded protocol is therefore used for all comparative results below, and Table~\ref{tab:ablation} reports the inference-time ablations under it. A fixed-threshold majority vote is degenerate there because nearly every per-sample confidence exceeds 0.5, so ranking-based evaluation uses the ensemble-mean confidence, of which the threshold-swept vote of Fig.~\ref{fig_res6} is one operating point.

Predictive value can only be demonstrated where occupancy changes. We therefore evaluate on the 135 validation windows whose 32-frame future contains at least one occupancy flip (Table~\ref{tab:comparison}), against baselines that share the same windows. Persistence copies a per-sub-block estimate of the current occupancy forward; in its deployable, sensed form the estimate comes from an FFT band-power detector or from the T2F classifier applied to the 128 history frames, and in its oracle form from the ground-truth history labels (AUC 0.8323). Direct label regression removes the generative stage and predicts the $32\times8$ occupancy map from the same frozen encoder context through a 4.7M-parameter head. A state-conditioned variant appends the 8-dimensional sensed occupancy to the diffusion context and adapts the diffusion stage alone. Finally, the fusion configuration takes the geometric mean of the proposed system's ensemble confidence and the sensed estimate.

Three findings follow. The generative stage on its own does not surpass sensed persistence (AUC 0.8185 versus 0.8499; direct regression 0.8382), with pairwise differences among these single-source configurations at or within the bootstrap noise level of about 0.01, and state conditioning moves the pipeline in the right direction ($+0.012 \pm 0.010$ AUC). Fusion, however, attains 0.8844 AUC and 0.9473 AP — $+0.034 \pm 0.008$ and $+0.044 \pm 0.007$ over sensed persistence ($P>0.999$) — approaching the 0.8967 bound of the T2F classifier applied to the true future waveform: the generated futures carry information about impending transitions that the sensed current state alone does not, and that information is realized when the two are combined. A fusion built on the direct-regression head reaches a statistically indistinguishable 0.8861 at far lower latency, so the gain reflects the complementarity of a learned predictor with a sensed prior rather than a property unique to diffusion. At the 1\% collision operating point relevant to secondary access, sensed persistence and fusion identify 12.8\% of vacant sub-blocks against 25.9\% for the oracle. With the 20 samples generated as a single batch, inference of the pipeline requires about 0.4~s per window on an A100 — essentially the cost of a single sample, since one sample already saturates the GPU, whereas the reference implementation, which samples sequentially, requires 7.5~s — more than four orders of magnitude above the 13~$\mu$s frame budget, which quantifies the engineering gap to deployment.
...
\begin{table*}[ht]
\centering
\caption{Stage ablations on the 135 validation windows whose 32-frame future contains an occupancy flip (seed 42; occupied base rate 0.690; bootstrap standard errors $\approx 0.01$). Configurations are defined in Section~\ref{sec:sys_eval}. AP: average precision (no-skill floor 0.690). Vacancy det.: fraction of vacant sub-blocks identified at the threshold that declares 1\% of occupied sub-blocks vacant. Latency: median single-window inference on one NVIDIA A100 with the 20 ensemble members generated as one batch (the reference implementation samples them sequentially and needs 7.5~s).}
\label{tab:comparison}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{3.4cm}|p{1.8cm}|p{1.2cm}|p{1.2cm}|p{2.8cm}|p{2.5cm}|}
\hline
\textbf{Configuration} & \textbf{Generative stage} & \textbf{AUC} & \textbf{AP} & \textbf{Vacancy det. @ 1\% collision} & \textbf{Latency / inference} \\
\hline

Sensed persistence (FFT energy detector)
  & \textit{none} & 0.8156 & 0.9039 & 6.6\% & 2.3 \ul{ms} \\
\hline

Sensed persistence (T2F on history)
  & \textit{none} & 0.8499 & 0.9031 & 12.8\% & 10 \ul{ms} \\
\hline

Encoder $\rightarrow$ CNN, direct label regression
  & \textit{removed} & 0.8382 & 0.9237 & 5.7\% & 43 \ul{ms} \\
\hline

Proposed (20 samples, ensemble mean)
  & full & 0.8185 & 0.9174 & 6.0\% & 410 \ul{ms} \\
\hline

Proposed, state-conditioned diffusion
  & full & 0.8301 & 0.9233 & 2.9\% & 412 \ul{ms} \\
\hline

\textbf{Proposed $\oplus$ sensed state (fusion)}
  & \textbf{full} & \textbf{0.8844} & \textbf{0.9473} & \textbf{12.8\%} & \textbf{417 \ul{ms}} \\
\hline

\textit{T2F on ground-truth waveforms (upper bound)}
  & \textit{oracle} & \textit{0.8967} & \textit{0.9146} & \textit{25.9\%} & --- \\
\hline
...
\begin{table}[ht]
\centering
\caption{Inference-time ablations under a seeded random validation draw (60 windows per environment, seed 42, $N{=}1800$; proposed configuration AUC 0.7612; see Section~\ref{sec:sys_eval}). $\Delta$~AUC is relative to the proposed configuration.}
\label{tab:ablation}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{|l|l|c|c|}
\hline
\textbf{Ablation} & \textbf{Setting} & \textbf{AUC} & \textbf{$\Delta$ AUC} \\
\hline
Ensemble size & 1 / 5 / 20 samples & 0.7449 / 0.7570 / 0.7612 & $-0.016$ / $-0.004$ / --- \\
\hline
Sampler & DDIM vs. DDPM (10 steps each) & 0.7711 & $+0.0099$ \\
\hline
Aggregation & majority vote (fixed 0.5) vs. mean probability & 0.5000 & $-0.2612$ \\
\hline
\end{tabular}%
}
\end{table}
...
\makecell{This work} & Generative Model & \makecell{\textbf{Raw} \\ \textbf{Signal Waveform} \\ $[1, t]$} & 1638.4 $\mu$s & \textbf{32} & \textbf{12.8} $\mu$s & \makecell{5.21 GHz -\\ 5.29 GHz} & 8 & \cmark \\
\hline
...
A first-order latency roadmap can be constructed from the measured breakdown of the 410~ms batched inference: 46~ms to encode the 128-frame history, about 35~ms per reverse diffusion step for the 20-sample batch (ten steps), and 10~ms to classify the 20 generated futures. Reducing the reverse chain to a single step, as few-step distillation and consistency models make possible, was measured at 79~ms end-to-end; caching per-frame embeddings so that only the newest frame is encoded per window would remove most of the encoder cost; compiler-level optimization with INT8/FP8 quantization typically yields a further factor of two to three at batch size one, a distilled student network for the 0.1B-parameter diffusion stage a factor of two to five, and current-generation accelerators a factor of two to three. Compounded, these established techniques project to roughly 2--7~ms per inference — two to two-and-a-half orders of magnitude below the reference implementation, yet still 5--15 times above the 410~$\mu$s prediction window and far above the 13~$\mu$s frame budget. Inference within the prediction window therefore additionally requires an order-of-magnitude reduction in model scale or dedicated hardware, and remains undemonstrated.

The diffusion model's probabilistic nature is well suited to this task because user arrivals and departures are inherently stochastic. Rather than producing a single deterministic forecast, the model generates multiple plausible future signals whose classifier confidences are aggregated. The ablations of Table~\ref{tab:comparison} locate where this generation earns its cost: on their own the sampled futures do not outperform a sensed estimate of the current occupancy, but fused with that estimate they approach the oracle bound on the windows where occupancy changes, indicating that the generative stage contributes information about impending transitions that is complementary to sensing. The contrastive-learning-based encoder is designed to generalize across diverse wireless environments by learning environment-agnostic representations. In this work, generalization is evaluated across the 30 simulated scenarios; transfer to real-world deployments with hardware impairments, unknown interference sources, or different air interface standards remains to be validated.
