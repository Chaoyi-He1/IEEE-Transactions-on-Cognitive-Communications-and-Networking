...
\begin{table*}[ht]
\centering
\caption{Stage ablations on the 135 transition-containing evaluation windows; protocol and definitions in Section~\ref{sec:sys_eval}.}
\label{tab:comparison}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{3.4cm}|p{1.8cm}|p{1.2cm}|p{1.2cm}|p{2.8cm}|}
\hline
\textbf{Configuration} & \textbf{Generative stage} & \textbf{AUC} & \textbf{AP} & \textbf{Vacancy det. @ 1\% collision} \\
\hline

Sensed persistence (FFT energy detector)
  & \textit{none} & 0.8156 & 0.9039 & 6.6\% \\
\hline

Sensed persistence (T2F on history)
  & \textit{none} & 0.8499 & 0.9031 & 12.8\% \\
\hline

Encoder $\rightarrow$ CNN, direct label regression
  & \textit{removed} & 0.8382 & 0.9237 & 5.7\% \\
\hline

Proposed (20 samples, ensemble mean)
  & full & 0.8185 & 0.9174 & 6.0\% \\
\hline

Proposed, state-conditioned diffusion
  & full & 0.8301 & 0.9233 & 2.9\% \\
\hline

\textbf{Proposed $\oplus$ sensed state (fusion)}
  & \textbf{full} & \textbf{0.8844} & \textbf{0.9473} & \textbf{12.8\%} \\
\hline
...
\begin{table}[ht]
\centering
\caption{Inference-time ablations of the fusion configuration on the transition-window protocol (Section~\ref{sec:sys_eval}); $\Delta$~AUC is relative to the proposed fusion (AUC 0.8844).}
\label{tab:ablation}
...
\hline
Ensemble size & 1 / 5 / 20 samples & 0.8830 / 0.8828 / 0.8844 & $-0.001$ / $-0.002$ / --- \\
\hline
Sampler & DDIM vs. DDPM (10 steps each) & 0.8895 & $+0.0051$ \\
\hline
Aggregation & majority vote (fixed 0.5) vs. mean probability & 0.8499 & $-0.0345$ \\
\hline
\end{tabular}%
}
\end{table}
