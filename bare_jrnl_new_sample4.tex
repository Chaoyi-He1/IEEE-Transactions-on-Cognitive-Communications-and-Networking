...
\begin{table*}[ht]
\centering
\caption{Stage ablations under the transition-window protocol: every row is evaluated on the 135 validation windows whose 32-frame future contains at least one occupancy flip (seed 42; occupied-class base rate 0.690; bootstrap standard errors $\approx 0.01$ for AUC and AP). Randomly drawn windows are excluded because occupancy dwell times far exceed the 410~$\mu$s horizon, so on the transition-free remainder of the corpus ($>$99\% of windows) the persistence baseline reproduces the future exactly by construction and no predictor can improve on it; the values quoted in Sections~\ref{sec:sys_eval} and~\ref{sec:zeroshot} refer to randomly drawn windows and are not comparable to this protocol. The two sensed-persistence rows replace the oracle's label access with a deployable sensing front end applied to the 128 history frames — an FFT band-power detector, or the T2F classifier itself — whose per-bin estimate is copied forward. Among single-source rows, T2F-sensed persistence attains the highest AUC and vacancy detection, while the direct-regression head and the proposed system attain the highest average precision, with the remaining single-source differences at or within the noise level (bootstrap standard errors 0.01--0.02). The fusion row combines the two complementary signals — the geometric mean of the proposed system's 20-sample ensemble confidence and the T2F-sensed history estimate — and attains the best non-oracle AUC and AP, exceeding sensed persistence by $+0.034 \pm 0.008$ (AUC) and $+0.044 \pm 0.007$ (AP), both $P>0.999$, and approaching the oracle bound; the diffusion samples therefore carry transition information that the sensed current state alone does not. An analogous fusion using the direct-regression head instead of the diffusion pipeline reaches a statistically indistinguishable AUC of 0.8861 at far lower latency. The state-conditioned row augments the diffusion conditioning with the 8-dimensional T2F-sensed history occupancy (a 264-dimensional context) and adapts the diffusion stage alone for 30 epochs with the encoder and classifier frozen; it improves on the unconditioned pipeline by $+0.012 \pm 0.010$ AUC, and its fusion with the sensed estimate reaches 0.8862 AUC / 0.9492 AP, statistically indistinguishable from the fusion row. AP is average precision (no-skill floor = 0.690). Vacancy det.\ @ 1\% collision is the fraction of genuinely vacant sub-blocks identified when the decision threshold is set so that 1\% of occupied sub-blocks are declared vacant. Latency is the median single-inference wall clock on one NVIDIA A100; the direct-regression head is a 4.7M-parameter network on the frozen encoder context. Zero-shot AP and vacancy scores were not retained for this protocol. The oracle row bounds the performance a perfect future waveform would allow.}
...
Persistence (oracle sensing: ground-truth history labels)
  & \textit{none} & 0.8323 & 0.8796 & 4.7\% & $<1$ $\mu$s \\
\hline
...
Proposed, state-conditioned diffusion (264-d context)
  & full & 0.8301 & 0.9233 & 2.9\% & 8068 \ul{ms} \\
\hline

\textbf{Proposed $\oplus$ sensed state (geometric-mean fusion)}
  & \textbf{full} & \textbf{0.8844} & \textbf{0.9473} & \textbf{12.8\%} & \textbf{8068 \ul{ms}} \\
\hline

Proposed, zero-shot (Sec. V-E)
  & full & 0.8004 & --- & --- & 8079 \ul{ms} \\
\hline
...
\begin{table}[ht]
\centering
\caption{Design-choice ablations on the saved system checkpoint. Each row varies one inference-time element; the AUC column reports the varied setting and $\Delta$~AUC is relative to the proposed configuration's 0.7612 measured on a random validation draw (60 windows per environment, seed 42, $N{=}1800$; a protocol distinct from the transition windows of Table~\ref{tab:comparison}). The fixed-threshold majority vote is degenerate at this operating point — nearly every per-sample confidence exceeds 0.5 — so threshold-free ranking requires the ensemble-mean confidence or a swept vote threshold as in Fig.~\ref{fig_res6}.}
\label{tab:ablation}
